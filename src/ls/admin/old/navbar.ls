  ...
  ldc.register \navbar, <[]>, ->
    prepare = (ctrl-opt) ->
      ...
      view-sample = new ldView do
        root: ld$.find(ctrl-opt.root, '[ld=folder-sample]', 0)
        handler: do
          item: (->), folder: (->)
      ...
      root-view = render-folder {node: ld$.find(ctrl-opt.root, '.folder-root', 0), data: tree}
      reb = new reblock do
        root: ld$.find(ctrl-opt.root, '.folder-root', 0)
        action: do
          beforeMove: (src, des) ->
