...
(function(){
  var startTime, fs, fsExtra, path, crypto, LiveScript, chokidar, moment, colors, express, bodyParser, expressSession, connectMultiparty, csurf, expressRateLimit, passport, passportLocal, passportFacebook, passportGoogleOauth20, nodemailer, sharedbWrapper, lderror, postgresql, api, ext, view, cache, aux, throttle, grecaptcha, action, watch, secret, mod, uglifyJs, lsc, modBuilder, customBuilder, backend;
  startTime = Date.now();
  fs = require('fs');
          ...
          this$.app.use(function(err, req, res, next){
            ...
            if (err.code === 'EBADCSRFTOKEN') {
              ...
              if (err.name === 'ldError' || err.name === 'lderror') {
                ...
                console.error(colors.red("[" + moment().format('YY/MM/DD HH:mm:ss') + "]"), colors.yellow(err.message), "[", color.yellow(req.originalUrl.substring(0, 15)), "]");
              } else if (err.message.startsWith('Failed to lookup view')) {
                console.error(colors.red("[" + moment().format('YY/MM/DD HH:mm:ss') + "]"), colors.yellow(err.message));
              } else {
                console.error(colors.red("[" + moment().format('YY/MM/DD HH:mm:ss') + "]"), colors.yellow(err.toString()), "[", colors.yellow(err.path || ''), "][", colors.yellow(req.originalUrl), "]");
