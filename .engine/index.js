...
(function(){
  var startTime, fs, fsExtra, path, crypto, LiveScript, chokidar, moment, express, bodyParser, expressSession, connectMultiparty, csurf, expressRateLimit, passport, passportLocal, passportFacebook, passportGoogleOauth20, nodemailer, sharedbWrapper, permcheck, lderror, postgresql, api, ext, view, permcache, aux, watch, secret, mod, uglifyJs, lsc, config, colors, modBuilder, customBuilder, backend;
  startTime = Date.now();
  fs = require('fs');
  ...
  ext = require('./ext');
  view = require('./view');
  permcache = require('./api/permcache');
  aux = require('./aux');
  watch = require('./watch');
        ...
        if (!config.debug) {
          this$.app.use(function(err, req, res, next){
            ...
            if (err.code === 'EBADCSRFTOKEN') {
              ...
              } else if (err.name === 'ldError' && in$(err.id, [1000])) {
