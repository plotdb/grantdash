...
ldc.register('flagship-form', ['loader', 'auth', 'error', 'viewLocals', 'ldcvmgr'], function(arg$){
  var loader, auth, error, viewLocals, ldcvmgr, lc, vlc, viewmode, lockform, init;
  loader = arg$.loader, auth = arg$.auth, error = arg$.error, viewLocals = arg$.viewLocals, ldcvmgr = arg$.ldcvmgr;
  loader.on();
  lc = {};
  vlc = viewLocals || {};
  console.log(vlc);
  ...
  lockform = debounce(100, function(lock){
    ...
    return ld$.find(form, "textarea,input,select").map(function(n, i){
      ...
      if (lock) {
        ...
        return n.removeAttribute('readonly');
      }
    });
  });
  init = function(arg$){
    ...
    budgetCalc = function(){
      ...
      ref$.self = self;
      ref$.percent = percent;
      cur = payload.budget.total && payload.budget.total > 0 && payload.budget.subsidy <= 5000000 && payload.budget.percent.subsidy <= 0.49;
    ...
    view = new ldView({
      ...
      handler: {
        "editmode-only": function(arg$){
          var node;
          node = arg$.node;
          return node.classList.toggle('d-none', viewmode);
        },
