...
(function(){
  return ldc.register('navbar', [], function(){
    var prepare;
    prepare = function(ctrlOpt){
      ...
      viewSample = new ldView({
        root: ld$.find(ctrlOpt.root, '[ld=folder-sample]', 0),
        handler: {
          item: function(){},
          folder: function(){}
        }
      ...
      rootView = renderFolder({
        node: ld$.find(ctrlOpt.root, '.folder-root', 0),
        data: tree
      });
      return reb = new reblock({
        root: ld$.find(ctrlOpt.root, '.folder-root', 0),
        action: {
